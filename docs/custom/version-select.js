  ...
  xhr.onload = function() {
    ...
    select.addEventListener("change", function(event) {
      ...
      if (match) { // path has version number in it
        href2 = window.location.href.replace(/\/v\/[0-9]*\.[0-9]*\.[0-9]*\//,  '/v/' + this.value + '/');
      } else { // path does not have version in it
        href2 = window.location.href.replace(window.location.origin, window.location.origin + '/v/' + this.value);
      }
